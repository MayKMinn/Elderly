    ...
    if valid-flag = "Y"
        if visit-date(5:1) not = "-"
            ...
            if visit-date(8:1) not = "-"
                ...
                if visit-date(1:4) is not numeric
                    move "N" to valid-flag
                    move "visitDate" to error-field
                    move "Enter visit date as YYYY-MM-DD." to error-message
                else
                    if visit-date(6:2) is not numeric
                        move "N" to valid-flag
                        move "visitDate" to error-field
                        move "Enter visit date as YYYY-MM-DD." to error-message
                    else
                        if visit-date(9:2) is not numeric
                            move "N" to valid-flag
                            move "visitDate" to error-field
                            move "Enter visit date as YYYY-MM-DD." to error-message
                        end-if
    ...
    if valid-flag = "Y"
        if function trim(allow-past) not = "Y"
            ...
            if visit-date-num < current-date-num
                move "N" to valid-flag
                move "visitDate" to error-field
                move "Visit date and time cannot be in the past." to error-message
            else
                if visit-date-num = current-date-num
...
validate-schedule-status.
    if valid-flag = "Y"
        if function trim(schedule-status) not = "scheduled"
            and function trim(schedule-status) not = "completed"
            and function trim(schedule-status) not = "missed"
            and function trim(schedule-status) not = "cancelled"
            move "N" to valid-flag
